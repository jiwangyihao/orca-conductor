...
| 并行完成若干任务，且每个任务都有足够体量、彼此需要足够隔离 | 各自有独立的目标与验收标准、各自需要独立的试错空间、**执行期间不需要看到对方的中间产物**；并行省下的时间明显大于三笔固定成本之和 |

第二条的两个"足够"是**同时**要求，缺一条就不成立：五个小任务并行，省下的时间通常抵不过五份 brief 的写作成本加五次回收。

但要分清"隔离不成立"到底指什么——**它指依赖，不指文件重叠**：

- **文件重叠不是否决理由。** 每个 worker 在自己的 worktree 里工作，执行期间既看不到也不会破坏别人的改动，同一批文件被两个大任务同时改**不会在执行期产生冲突**。代价只出现在最后把成果合并回协调工作区那一步，而**合并本来就是 coordinator 的份内工作**，属于自然必要的开销，不该被算作"并行不划算"的证据。你要做的是在 S1 就把合并策略写下来（合并顺序、以谁为基准、冲突时以哪条 DoD 判优），而不是因为怕合并就改成串行。
- **真正让隔离不成立的是依赖**：B 需要 A 的产物或结论作为输入、两者必须协商同一处接口设计、或必须共享执行期的中间状态。这类情况的问题不是"会冲突"，而是下游只能基于猜测工作——正确做法是**切 Phase 串行**，把 A 的产物固化成 commit 当作 B 的入口基线。
- 唯一需要预警的重叠情形：两个 worker 会各自重写同一处代码的整体结构（比如都重构同一个模块的骨架）。此时合并可能退化成"二选一，丢掉另一份"，并行收益被合并阶段吃掉。判断方式是估合并成本、和串行做对比，必要时先派一个 worker 固化接口并 commit，再并行推进其余部分——这仍然是成本核算，不是硬禁止。
